...
namespace Base
{
    ...
    public class Funcionalidad : ComunBase
    {
        private List<Transferencia.Funcionalidad> lista;

        public Funcionalidad(string rutaArchivo) : base(rutaArchivo)
        {
            this.Carga();
        }
        ...
        public bool Existe(Transferencia.Funcionalidad objeto)
        {
            return this.lista != null ? !(this.lista.FindIndex(e => e.Id == objeto.Id) < 0) : false;
        }
        public void Guardar(Transferencia.Funcionalidad objeto)
        {
        ...
        private long DeterminaSiguiente()
        {
            ...
            foreach (Transferencia.Funcionalidad tmp in this.lista)
            {
                if (retorno < tmp.Id)
                {
                    retorno = tmp.Id;
        ...
        protected void Carga()
        {
            if (new System.IO.FileInfo(this.rutaArchivo).Exists)
            {
                ...
                this.lista = Util.Operacion.StringToObject<List<Transferencia.Funcionalidad>>(xml);
            }
            else
            {
                this.lista = new List<Transferencia.Funcionalidad>();
                this.ActualizaOrigen();
            }
        }
    }
