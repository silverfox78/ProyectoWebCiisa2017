...
namespace Base
{
    ...
    public class SubTipo : ComunBase
    {
        private List<Transferencia.SubTipo> lista;

        public SubTipo(string rutaArchivo) : base(rutaArchivo)
        {
            this.Carga();
        }
        ...
        public bool Existe(Transferencia.SubTipo objeto)
        {
            return this.lista != null ? !(this.lista.FindIndex(e => e.Id == objeto.Id) < 0) : false;
        }

        public void Guardar(Transferencia.SubTipo objeto)
        {
        ...
        private long DeterminaSiguiente()
        {
            ...
            foreach (Transferencia.SubTipo tmp in this.lista)
            {
                if (retorno < tmp.Id)
                {
                    retorno = tmp.Id;
        ...
        protected void Carga()
        {
            if (new System.IO.FileInfo(this.rutaArchivo).Exists)
            {
                ...
                this.lista = Util.Operacion.StringToObject<List<Transferencia.SubTipo>>(xml);
            }
            else
            {
                this.lista = new List<Transferencia.SubTipo>();
                this.ActualizaOrigen();
            }
        }
    }
